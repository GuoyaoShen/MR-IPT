"""MR-IPT model and supporting CNN blocks."""
...
class MRIPT(nn.Module):
    """MR-IPT reconstruction model with head/body/tail branches."""
    ...
    def forward(self, x, levels, types):
        ...
        head_out = torch.zeros(
            (x.shape[0], self.n_feats, x.shape[-2], x.shape[-1]), device=self.device
        )
        for i in range(x.shape[0]):
            if self.mode == "combine":
                level_i = levels[i]
                type_i = types[i]
                head_out[i] = self.head[type_i][level_i](
                    x[i].unsqueeze(0).to(self.device)
                ).squeeze(0)
            elif self.mode == "level":
                level_i = levels[i]
                head_out[i] = self.head[level_i](
                    x[i].unsqueeze(0).to(self.device)
                ).squeeze(0)
            elif self.mode == "type":
                type_i = types[i]
                head_out[i] = self.head[type_i](
                    x[i].unsqueeze(0).to(self.device)
                ).squeeze(0)

        image_embeddings = self.image_encoder(head_out)
        sparse_embeddings = self.prompt_encoder(levels=levels, types=types)
        image_decodings = self.image_decoder(
        ...
        tail_out = torch.zeros(
            (x.shape[0], self.n_colors, x.shape[-2], x.shape[-1]), device=self.device
        )
        for i in range(res.shape[0]):
            if self.mode == "combine":
                level_i = levels[i]
                type_i = types[i]
                tail_out[i] = self.tail[type_i][level_i](
                    res[i].unsqueeze(0).to(self.device)
                ).squeeze(0)
            elif self.mode == "level":
                level_i = levels[i]
                tail_out[i] = self.tail[level_i](
                    res[i].unsqueeze(0).to(self.device)
                ).squeeze(0)
            elif self.mode == "type":
                type_i = types[i]
                tail_out[i] = self.tail[type_i](
                    res[i].unsqueeze(0).to(self.device)
                ).squeeze(0)
