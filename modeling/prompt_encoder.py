"""Prompt encoder modules for MR-IPT.

Code adapted from Segment Anything prompt encoding.
"""
...
class PromptEncoder(nn.Module):
    """Encode per-head prompts into sparse token embeddings."""
    ...
    def __init__(
        ...
        head_embeddings = [
            nn.Embedding(1, embed_dim) for _ in range(self.num_head_embeddings)
        ]
    ...
    def _embed_heads(
        ...
        """Embeds head prompts."""
        head_embedding = torch.zeros(
            (labels.shape[0], 1, self.embed_dim), device=labels.device
        )
...
class PromptEncoderMulti(nn.Module):
    """Encode level and type prompts into sparse token embeddings."""
    ...
    def __init__(
        ...
        level_embeddings = [
            nn.Embedding(1, embed_dim) for _ in range(self.num_level_embeddings)
        ]
        self.level_embeddings = nn.ModuleList(level_embeddings)

        self.num_type_embeddings = num_type
        type_embeddings = [
            nn.Embedding(1, embed_dim) for _ in range(self.num_type_embeddings)
        ]
    ...
    def _embed_levels(
        ...
        """Embeds head prompts."""
        level_embedding = torch.zeros(
            (levels.shape[0], 1, self.embed_dim), device=levels.device
        )
    ...
    def _embed_types(
        ...
        """Embeds head prompts."""
        type_embedding = torch.zeros(
            (types.shape[0], 1, self.embed_dim), device=types.device
        )
...
def get_position_encoding_given_key(d, k, n=None):
    """Return sinusoidal positional encoding for index k and dimension d."""
    if n is None:
        n = torch.tensor([10000.0])
    P = torch.zeros(d)
